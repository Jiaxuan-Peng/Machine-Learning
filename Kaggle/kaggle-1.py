...
train = pd.read_csv('D:/utah/courses/y1s1/ml/kaggle/train_final.csv', sep=',', header=0)
test = pd.read_csv('D:/utah/courses/y1s1/ml/kaggle/test_final.csv', sep=',', header=0)
...
#rf = GridSearchCV(rf, {'n_estimators': [100, 200, 500],'max_depth': [1, 5,10]}, verbose=1,n_jobs=2,cv=10)
...
#ada = GridSearchCV(ada, {'n_estimators': [100, 200, 500]}, verbose=1,n_jobs=2,cv=10)
...
#ann
ann = MLPClassifier(hidden_layer_sizes = 20, random_state=1)
#ann = GridSearchCV(ann, {'hidden_layer_sizes': [10, 20, 50, 100, 200]}, verbose=1,n_jobs=2,cv=10)
...
#xgb
xgb = xgb.XGBClassifier(max_depth=10,n_estimators=500)
#xgb = GridSearchCV(xgb, {'max_depth': [10, 15, 20],'n_estimators': [100, 200,500]}, verbose=1,n_jobs=2,cv=10)
...
#gbt = GridSearchCV(gbt, {'n_estimators': [100, 150, 200],'max_depth': [1, 5,10]}, verbose=1,n_jobs=2,cv=10)

#apply cross - validation for hyper - parameter selection
#score = cross_val_score(gbt,x_train,y_train,cv=10)
#score.mean()
